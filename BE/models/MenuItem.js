...
menuItemSchema.methods.incrementTerjual = async function (hari, jumlah) {
    ...
    const dayQuota = this.currentWeekSchedule.dailyQuotas.find(
        (dq) => dq.hari === hari
    );
    ...
    if (available < jumlah) {
        throw new Error(
            `Quota tidak cukup untuk ${this.nama} di ${hari}. Tersedia: ${available}, diminta: ${jumlah}`
        );
    }
...
menuItemSchema.methods.setCurrentWeekSchedule = async function (
    weekNumber,
    year,
    dailyQuotas
) {
...
menuItemSchema.statics.getMenusForDay = async function (
    weekNumber,
    year,
    hari
) {
